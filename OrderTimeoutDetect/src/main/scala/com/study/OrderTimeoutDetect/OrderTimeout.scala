package com.study.OrderTimeoutDetect
...
object OrderTimeout {
  def main(args: Array[String]): Unit = {
    ...
    val orderPayPattern = Pattern.begin[OrderEvent]("begin").where(_.eventType =="create")
      ...
      val completeResultDataStream =patternStream.select(orderTimeOutput)(
      ...
      ( pattern: Map[String, Iterable[OrderEvent]],timestamp:Long)=> {
        ...
        ( pattern: Map[String, Iterable[OrderEvent]]) => {
          val payedOrderId =pattern.getOrElse("begin",null).iterator.next().orderId
          OrderResult(payedOrderId,"success")
        }
      )
    completeResultDataStream.print()  //打印出来的是匹配的事件序列

    //打印输出timeout结果
    val timeoutResultDataStream=completeResultDataStream.getSideOutput(orderTimeOutput)
